...
| 🛡️ **多 Provider 配置安全** | 多模型配置的隔离、真实推理验证、fallback 与回滚门禁。 | `multi-provider-config-safety` |
| ✅ **GitHub 仓库质量门** | 原子变更、分支迁移、内容验证与最终 SHA 对齐的 CI 验收。 | `github-repo-quality-gate` |
| 📦 **异步工件投递状态机** | 幂等键、不可变工件、pending/uncertain 状态与收件确认。 | `async-artifact-delivery-state-machine` |
| ⏱️ **定时模型故障恢复** | cron 模型故障分级、真实推理验证、最小路由修复与回滚。 | `scheduled-model-failure-recovery` |
